...
with $`BAF`$ being the basal area factor (in m$`^2`$/ha) and $`g_i`$ the
...
$`z`$ coordinate delimited by Voronoi polygons ($`h_{P_{99}i}`$, in m).
The equation used for the calculation for circular fixed area and k-tree
plots is
...
The above mentioned calculations of the stand-level variables $`N`$,
$`V`$ and $`G`$ do not consider possible occlusions of trees. Therefore,
...
Strahler et al. (2008[^1]) and Lovell et al. (2011[^2]).
...
more details see Seidel and Ammer (2014[^3]).
...
$`n`$ largest trees per hectar are considered. If not otherwise
specified in the argument `num.tree` (see above), the number of dominant
trees per hectare is set to 100 trees/ha. Dominant heights and diameters
are also calculated as the arithmetic (**`h.0.tls`**, **`d.0.tls`**),
quadratic (**`h.0.g`**, **`d.0.g`**), geometric (**`h.0.geom`**,
**`d.0.geom`**) and harmonic means (**`h.0.harm`**, **`d.0.harm`**).

[^1]: Strahler, A.H., Jupp, D.L.B., Woodcock, C.E., Schaaf, C.B., Yao,
...
[^3]: Seidel, D., Ammer, C., 2014. Efficient measurements of basal area
    in short rotation forests based on terrestrial laser scanning under
