...
tree.detection.single.scan_opt_par <- function(data, dbh.min = 4, dbh.max = 200, h.min = 1.3,
  ...
  .filteraux <-  foreach(cuts = breaks, .combine=rbind) %dopar%{
  ...
  # for(cuts in breaks){
    ...
    #
    # .filter <- do.call(rbind, parLapply(cl, split(.cut, .cut$cluster), .sections.single.scan,
    #                                  .alpha.v = .alpha.v, .alpha.h = .alpha.h,
    #                                  .dbh.min = .dbh.min, .dbh.max = .dbh.max))
    #
  ...
  if(nrow(.filter) < 1) {
    ...
    eje <- .filter[.filter$sec < ((stem.section[2] - stem.section[1]) / 2) + 0.5 &
                     .filter$sec > ((stem.section[2] - stem.section[1]) / 2) - 0.5,
                   c("center.x", "center.y", "center.rho", "center.phi", "radius", "sec")]
    .dbscan <- dbscan::dbscan(eje[, c("center.x", "center.y"), drop = FALSE], eps = max(eje$radius), minPts = 1)
