...
tree.detection.multi.scan_2 <- function(data, single.tree = NULL,
  ...
  # data <- rbind(data[data$z < stem.section[1] | data$z > stem.section[2], ], stem)
  ...
  if(is.null(tls.precision)){
  dat <- VoxR::vox(dat[, c("x", "y", "z")], res = 0.03)} else {
    dat <- VoxR::vox(dat[, c("x", "y", "z")], res = tls.precision)}
  ...
  points(median(stem$x), median(stem$y), col = "red", pch = 19)
  ...
  points(stem$x, stem$y, pch = 19, cex = 0.1, col = "blue")
  ...
  for (i in unique(.filteraux$tree)) {
    ...
    # .filt <- .filt[.filt$dist < mean(.filt$dist, na.rm = TRUE), ]
