...
sim.fix <- simulations$fixed.area
...
```{r, echo=FALSE}
#shiny::includeHTML("correlations.h.fixed.area.pearson.html")
#htmltools::includeHTML("correlations.h.fixed.area.pearson.html")
#htmltools::tags$iframe(title = "My embedded document", src = "correlations.h.fixed.area.pearson.html")
```
...
```{r}
cor$correlations$pearson$fixed.area[1:6,1:10]
```

All mentioned data frames are divided into rows each of which represent a given plot size. 

```{r}
cor$correlations.pval$pearson$fixed.area[1:6,1:10]
```

```{r}
head(cor$opt.correlations$pearson$fixed.area)
```

```{r eval=FALSE}
optimize.plot.design(correlations = cor$correlations,
                     variables = c("N", "G", "d", "dg", "d.0", "h", "h.0"),
                     dir.result = NULL)
```
