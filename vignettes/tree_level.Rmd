...
```{r include=FALSE}
dir.data <- system.file("extdata", "Galicia_multi_scan.laz", package="FORTLS")
dir.result <- system.file("extdata", "Galicia_multi_scan.laz", package="FORTLS")
...
system.file("exdata", "Galicia_multi_scan.laz", package = "FORTLS")
pcd.single <- normalize(las = paste(dir.data, "/Galicia_multi_scan.laz", sep = ""), 
...
Furthermore the scanning approach applied for data collection must be specified in **`scan.approach`** with `"single"` (set by default) indicating the TLS single-scan approach and `"multi"` indicating the TLS multi-scan and SLAM point clouds approaches.
...
The data frame is saved as .csv file without row names using the `write.csv` function from the [utils](https://cran.rstudio.com/web/packages/R.utils/index.html) package.
...
```{r eval=FALSE, include=TRUE}
tree.lists.sev.tls <- tree.detection.several.plots(
                            las.list = c("Galicia_multi_scan.laz", "Fagus_sylvatica_SLAM.laz"), 
                            id = NULL, file = ("multi.txt", "SLAM.txt"),
