...
// Fixed radius neighbors function
// [[Rcpp::export]]
List fixed_radius_neighbors_with_ids(NumericMatrix mat, double eps) {
  int n = mat.nrow();
  List neighbors(n);
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < n; ++j) {
      ...
      double dx = mat(j, 1) - x_i;
      double dy = mat(j, 2) - y_i;
      double dz = mat(j, 3) - z_i;

      double dist = std::sqrt(dx*dx + dy*dy + dz*dz);

      if (dist <= eps) {
        neighbor_points.push_back(static_cast<int>(mat(j, 0)));  // cast point ID to int
      }
    }

    neighbors[i] = wrap(neighbor_points);  // store integer vector
  }
...
// [[Rcpp::export]]
std::map<std::string, double> geometric_features_point_parallel(const Eigen::Map<Eigen::VectorXd>& x,
                                                                const Eigen::Map<Eigen::VectorXd>& y,
                                                                const Eigen::Map<Eigen::VectorXd>& z,
                                                                int pto,
                                                                double dist,
...
DataFrame geometric_features(const Eigen::MatrixXd& m,
  ...
  for (q = 0; q < n; ++q) {
    ...
    if (!(x_map[q] > lastX - dist || x_map[q] < firstX + dist || y_map[q] > lastY - dist || y_map[q] < firstY + dist || z_map[q] > lastZ - dist || z_map[q] < firstZ + dist)) {
     ...
     std::map<std::string, double> features = geometric_features_point_parallel(x_map, y_map, z_map, q, dist,
